...
import { calculateSongProgress, calculateSongProgressBefore24h } from '../../../lib/songs';
...
const SongHeader: React.FC<SongHeaderProps> = ({ song }) => {
	...
	const { updateSongOnServer } = useSongs();
	...
	const handleSaveSong = async (payload: { song: Song; imageFile?: File | null; audioFile?: File | null }) => {
		setIsSaving(true);
		try {
		...
		<>
			<div className="song-header">
				<div className="header-left">
					...
					<div className="header-progress-section">
						...
						<ProgressBar new_value={newProgress} old_value={oldProgress} />
					</div>
				</div>
			...
			<div className="header-actions">
				<Button
					...
					variant="primary"
					size="lg"
					className="header-guided-practice-btn"
					onClick={() => {
						// Guided Practice functionality to be implemented
					}}
				/>
				<div className="header-action-play">
					...
					<div className="header-action-archive">
						<IconButton
							Icon={song.archived ? UnarchiveOutlinedIcon : ArchiveOutlinedIcon}
